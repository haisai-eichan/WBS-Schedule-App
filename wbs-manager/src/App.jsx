...
function WBSEditor({ project, onProjectUpdate, onBack }) {
    ...
    const handleTaskUpdate = (taskId, updates, parentTasks = tasks) => {
        ...
        let newTasks = updateTask(parentTasks)

        // スケジュール連動: 開始日または期間が変更された場合、後続タスクを調整
        if ('startDate' in updates || 'duration' in updates) {
            newTasks = cascadeScheduleUpdate(taskId, newTasks)
        }
    ...
    // カスケードスケジュール更新
    const cascadeScheduleUpdate = (changedTaskId, taskList) => {
        const updateSiblings = (siblings) => {
            let previousEndDate = null
            let foundChangedTask = false

            return siblings.map((task) => {
                // 変更されたタスクを見つけたらフラグを立てる
                if (task.id === changedTaskId) {
                    foundChangedTask = true
                    if (task.endDate) {
                        previousEndDate = task.endDate
                    }
                    console.log('[CASCADE] Found changed task:', task.name, 'endDate:', task.endDate)
                    return task
                }

                // 変更されたタスクの後で、autoLinkが有効な場合のみ更新
                if (foundChangedTask && previousEndDate && task.autoLink !== false) {
                    const newStartDate = getNextBusinessDay(previousEndDate)
                    const duration = task.duration || 0
                    const newEndDate = calculateEndDate(newStartDate, duration)

                    console.log('[CASCADE] Updating task:', task.name, 'from', task.startDate, 'to', newStartDate)
                ...
                // 現在のタスクの終了日を記録（変更前のタスクの場合）
                if (!foundChangedTask && task.endDate) {
                    previousEndDate = task.endDate
                }

                // foundChangedTask後で、autoLink=falseの場合は終了日を更新
                if (foundChangedTask && task.endDate) {
                    previousEndDate = task.endDate
                }
        ...
        const processLevel = (items) => {
            const updated = updateSiblings(items)
            return updated.map(task => {
                if (task.children && task.children.length > 0) {
                    return { ...task, children: processLevel(task.children) }
                }
                return task
            })
        }

        console.log('[CASCADE] Starting cascade update for task ID:', changedTaskId)
        return processLevel(taskList)
    }
    ...
    const getRemainingBusinessDays = () => {
        const endDate = project.scheduledEndDate || getCalculatedEndDate()
        if (!endDate) return null

        const today = new Date().toISOString().split('T')[0]
        const remaining = calculateBusinessDays(today, endDate)

        return {
            days: remaining,
            urgency: remaining < 10 ? 'urgent' : remaining < 30 ? 'warning' : 'normal'
        }
    }
                    ...
                    {(() => {
                        ...
                        return (
                            <div className="countdown-section">
                                <div className="countdown-label">📅 残り営業日</div>
                                <div className={`countdown-value countdown-${remaining.urgency}`}>
                                    {remaining.days}
            ...
            <div ref={setNodeRef} style={style} className={`task-row level-${level} ${task.status === 'completed' ? 'task-row-completed' : ''}`}>
                <div className="task-main">
                    ...
                    <div className="task-actions">
                        ...
                        {/* スケジュール連動チェックボックス */}
                        {canEdit && (
                            <label className="autolink-checkbox" title="スケジュール連動">
                                <input
                                    type="checkbox"
                                    checked={task.autoLink !== false}
                                    onChange={(e) => handleTaskUpdate(task.id, { autoLink: e.target.checked })}
                                />
